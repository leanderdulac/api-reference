...
---|---|---
`api_key`<br />**obrigatório** | `String` | Chave da API
`id`<br />**obrigatório** | `Integer` | id da transação
`bank_account_id`<br />**obrigatório** | `Integer` | Se você tiver o id de uma conta previamente criada, você pode passar apenas seu id. Caso a conta ainda não exista, você pode [criar uma conta](https://docs.pagar.me/#criando-uma-conta-bancaria) ou passar os dados da conta via parâmetros
`bank_code`<br />**obrigatório** | `Integer` | Dígitos que identificam cada banco. Confira a lista dos bancos [aqui](http://www.febraban.org.br/arquivo/bancos/sitebancos2-0.asp)
`agencia`<br />**obrigatório** | `String` | Número da agência bancária
`agencia_dv`<br /> | `String` | Digito verificador da agência. Obrigatório caso o banco o utilize
...
---|---|---
`api_key`<br />**obrigatório** | `String` | Chave da API
`max_installments` | `Integer` | Valor máximo de parcelas<br />**Valor padrão: **12
`free_installments` | `Integer` | Número de parcelas isentas de juros<br />**Valor padrão: **1
`interest_rate`<br />**obrigatório** | `Double` | Valor da taxa de juros
`amount`<br />**obrigatório** | `Integer` | Valor do produto/serviço vendido
...
`ip` | `String` | IP de onde o request foi originado
